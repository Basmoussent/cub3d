...
{
	if (game->key_bool[0])
		move(game, game->p->pos_x + game->p->dir_x * MOVESPEED, game->p->pos_y + game->p->dir_y * MOVESPEED);
	if (game->key_bool[1])
		move(game, game->p->pos_x - game->p->dir_x * MOVESPEED, game->p->pos_y - game->p->dir_y * MOVESPEED);
	if (game->key_bool[2])
		move(game, game->p->pos_x + game->p->dir_y * MOVESPEED, game->p->pos_y - game->p->dir_x * MOVESPEED);
	if (game->key_bool[3])
		move(game, game->p->pos_x - game->p->dir_y * MOVESPEED, game->p->pos_y + game->p->dir_x * MOVESPEED);
}
